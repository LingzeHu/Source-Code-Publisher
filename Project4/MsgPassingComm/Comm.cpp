...
{
	std::string db1(rwBuffer, static_cast<size_t>(blockSize));
	std::cout << "\n Sending File Block:";
	std::cout << "\n --------------------------------------\n";
	std::cout << db1;
	std::cout << "\n End of File Block";
...
bool Sender::sendFile(Message msg)
{
	...
	while (true)
	{
		...
		connecter.send(static_cast<size_t>(blockSize), rwBuffer);
		if (blockSize == 0)
			break;
	}
	sendFile.close();
		...
		*/
		...
		bool receiveFile(Message msg)
		{
			...
			while (true)
			{
				...
				pSocket->recv(static_cast<size_t>(blockSize), rwBuffer);
...
int main()
{
	...
	DemoSndrRcvr("D:\CSE687\Project4\code\Pr4\Project4HelpWPF\Storage");  // replace "Odin" with your machine name or localhost
	//DemoCommClass("Odin");
	//DemoClientServer();
