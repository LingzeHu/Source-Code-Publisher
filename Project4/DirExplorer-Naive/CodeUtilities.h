...
namespace Utilities
{
	...
	public:
		...
		void appPath(const AppPath& appPath);
		Options options();
	...
	inline void ProcessCmdLine::appPath(const AppPath& appPath)
	{
		appPath_ = appPath;
	}


	inline ProcessCmdLine::AppPath ProcessCmdLine::appPath()
	{
		return appPath_;
	}

	inline void ProcessCmdLine::showAppPath()
	{
		out_ << appPath_ << " ";
	}
	/*----< options operations >---------------------------------------*/
	...
	{
		if (appPath_ == "" && path_ == "" && options_.size() == 0 && patterns_.size() == 0)
		{
			out_ << "empty";
			return;
		}
	...
	template <typename T>
	std::string Converter_<T>::toString(const T& t)
	{
	...
	*/
	template<typename T>
	T Converter_<T>::toValue(const std::string& src)
	{
