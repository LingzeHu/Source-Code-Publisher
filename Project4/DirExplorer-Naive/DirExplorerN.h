...
namespace FileSystem
{
	...
	inline void DirExplorerN::search1()
	{
		getFileList(path_);
	}
	...
	*/
	inline void DirExplorerN::find(const std::string& path)
	{
		...
		//if (path == "..")
		//	fpath = FileSystem::Path::getFullFileSpec(path);
		//else
		//	fpath = path;
	...
	inline void DirExplorerN::getFileList(const std::string& path)
	{
		/*std::vector<std::string> fileList;
		std::string filepath;*/
		std::string fpath = FileSystem::Path::getFullFileSpec(path);;
		...
		for (auto d : dirs)
		{
			...
			if (recurse_)
			{
				getFileList(dpath);   // recurse into subdirectories
			}
			else
			{
				doDir(dpath);  // show subdirectories
